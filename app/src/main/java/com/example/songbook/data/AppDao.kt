...
interface AppDao {
    ...
    @Query("SELECT * FROM song_table WHERE bandName LIKE '%' || :searchQuery || '%' AND isFavorite = 1 ORDER BY bandName")
    fun getFavoriteBands(searchQuery: String) : Flow<List<BandWithSongs>>

    @Query("SELECT * FROM song_table WHERE songName LIKE '%' || :searchQuery || '%' AND isFavorite = 1 ORDER BY songName")
    fun getFavoriteSongs(searchQuery: String) : Flow<List<Song>>
